/* worker.js (TEMPLATE) -- runs the engine off the main thread so the UI never janks.
 *
 ...
 *
 * Residual panels are cached per (backbone, AR, W) inside engine.compute, so only
 * changes to those trigger a regression pass; k/scheme/cap/unit-set re-aggregate
 * near-instantly.
 */
"use strict";
...
const PANELS = {};
const CACHE = new Map();
const CACHE_MAX = 24;                       // each entry ~1 MB; bound it
...
onmessage = e => {
  ...
  try {
    if (msg.type === "init") {
      for (const [name, p] of Object.entries(msg.panels || {})) {
        if (p && p.inflation && p.weights)
          PANELS[name] = ISMEngine.toPanel(p.inflation, p.weights);
      }
    ...
    if (msg.type === "compute") {
      ...
      postMessage({ type: "result", id: msg.id, backbone: msg.backbone,
                    Index: res.Index, S_pos: res.S_pos, S_neg: res.S_neg,
                    drivers: res.drivers, ms: Date.now() - t0 });
    }
  } catch (err) {
